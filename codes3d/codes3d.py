...
def parse_args():
    ...
    parser.add_argument(
        '-fs', '--features', action='store_true', default=False, help='epigenetic features that overlaps with the input SNPs')
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    if args.snp_input or args.snps_within_gene:
        ...
        if args.features:
            snp_df = features.feature_intersect(snp_df, args.output_dir, logger)
            snp_df.to_csv(os.path.join(args.output_dir, 'snps_features.txt'), sep = '\t', index = False) 
            snp_df = snp_df[(snp_df.iloc[:, 7:] == True).any(axis=1)]
